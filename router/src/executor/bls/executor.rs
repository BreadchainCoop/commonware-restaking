    ...
    for BlsEigenlayerExecutor<H>
{
    async fn execute_verification(
        ...
        let signatures: Vec<commonware_avs_core::bn254::Signature> = verification_data
            .signatures
            .iter()
            .map(|bytes| {
                commonware_avs_core::bn254::Signature::try_from(bytes.as_slice())
                    .map_err(|e| anyhow::anyhow!("Failed to deserialize signature: {:?}", e))
            })
            .collect::<Result<Vec<_>>>()?;

        // Deserialize public keys
        let public_keys: Vec<commonware_avs_core::bn254::PublicKey> = verification_data
            .public_keys
            .iter()
            .map(|bytes| {
                commonware_avs_core::bn254::PublicKey::try_from(bytes.as_slice())
        ...
        let g1_public_keys = if let Some(context) = verification_data.context {
            ...
            for chunk in context.chunks(G1_COMPRESSED_SIZE) {
                let g1_pubkey = commonware_avs_core::bn254::G1PublicKey::try_from(chunk)
    ...
    for BlsEigenlayerExecutor<H>
{
    async fn execute_bls_verification(
        ...
        let (_apk, _apk_g2, asig) =
            commonware_avs_core::bn254::get_points(participating_g1, participating, signatures)
